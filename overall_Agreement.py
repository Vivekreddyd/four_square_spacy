...
path = '/home/vivek/Desktop/Thesis/Four_Square_tweets/Adjudicated/agreements'
for root, dir1,file in os.walk(path):
    if (not file==[]):
        input1=os.path.join(root,file[0])
        input2=os.path.join(root,file[1])
...
for val in range(3,8):
    temp_X.extend(dict_x[val])
    temp_Y.extend(dict_y[val])
print(cohen_kappa_score(temp_X, temp_Y))
for indx,x in enumerate(temp_X):
    alpha_X.append([0,str(indx),x])
for indx,y in enumerate(temp_Y):
    alpha_Y.append([1,str(indx),y])
taskdata1=list(alpha_X)+list(alpha_Y)
# for k in range(0,2):
#     for j in range(3,8):
#         temp=(j - 3)* len(dict_x[j])
#         for i in range(0,len(dict_x[j])):
#             taskdata.append([k, str(temp+i), int(dict_x[j][i])])
ratingtask = agreement.AnnotationTask(data=taskdata1, distance=dist)
print("alpha " +str(ratingtask.alpha()))
